			...
			<tbody>
				...
				<tr class="hover:bg-blue-50 transition">
					...
					<td class="px-5 text-center py-3 border-b space-y-1">
						...
						@else
							<!-- TOMBOL TERIMA -->
							<form action="{{ route('terima', $surat->id_surat) }}" method="POST" class="inline-block">
								@csrf
								<button type="submit" class="bg-green-500 hover:bg-green-600 text-white text-sm px-4 py-1 rounded">Terima</button>
							</form>

							<!-- TOMBOL TOLAK -->
							<form action="{{ route('tolak', $surat->id_surat) }}" method="POST" class="inline-block">
								@csrf
						...
						<!-- TOMBOL PREVIEW -->
						<button onclick="lihatDetail({{ $surat->id }})"
